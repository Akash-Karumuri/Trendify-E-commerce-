...
const AddShirt = () => {
   ...
   const AddNewShirt=(e)=>{
    e.preventDefault();
    axios.post(`http://localhost:4000/Shirts` ,{name,description,price,originalPrice,discount,image,material,colors,sizes,rating})
    .then((res) => {
    ...
    <div className='container col-lg-6 mx-auto shadow p-5 m-5'>
      <h2 className='text-center'>Add New Shirt</h2>
      <form onSubmit={AddNewShirt}>
        <label>Shirt Name:</label>
        <input value={name} onChange={(e) => setName(e.target.value)} type="text" id="name" name="name" placeholder="Enter Shirt Name" className='form-control' required />
        <label>Description:</label>
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} id="description" name="description" placeholder="Enter Shirt Description" className='form-control' required />
        <label>Price:</label>
        ...
        <div className='flexcenter'><button type="submit" className="btn btn-primary mt-3">Submit</button></div>
      </form>
    </div>
  )
}
