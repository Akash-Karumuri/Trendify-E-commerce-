...
const AddShoe = () => {
   ...
   const AddNewShoe = (e) => {
    e.preventDefault();
    axios.post(`http://localhost:4000/Shoes`, {name, description, price, originalPrice, discount, image, material, colors, sizes, rating})
    .then((res) => {
  ...
  return (
    <div className='container col-lg-6 mx-auto shadow p-5 m-5'>
      <h2 className='text-center'>Add New Shoe</h2>
      <form onSubmit={AddNewShoe}>
        <label>Shoe Name:</label>
        <input value={name} onChange={(e) => setName(e.target.value)} type="text" id="name" name="name" placeholder="Enter Shoe Name" className='form-control' required />
        
        <label>Description:</label>
        <textarea value={description} onChange={(e) => setDescription(e.target.value)} id="description" name="description" placeholder="Enter Shoe Description" className='form-control' required />
